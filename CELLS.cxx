...
int main()
{
    ...
    SDL_FillRect(screenSurface, NULL, SDL_MapRGB(screenSurface->format, 0xAFF, 0xAFF, 0xAFF));
    ...
    for (int cycle = 0;; cycle++)
    {
        ...
        for (int i = 0; i < size; i++)
        {
            for (int j = 0; j < size; j++)
            {
                ...
                rect.h=0;
                rect.w=0;
                //const SDL_Rect *silka=rect;
                SDL_FillRect(screenSurface, &rect, SDL_MapRGB(screenSurface->format,0xFF,world[i][j].energy,0xFF));
            }
            cout << endl;
        }
